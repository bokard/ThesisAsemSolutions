...
filter_window=2 # Time for the rolling average filter in seconds
...
def process_talker_positions(data, companion_sock):
    ...
    try:
        for i in range(num_positions):
            ...
            if any(active_areas):
                # Remove areas older than 2 seconds
                active_areas = [area for area in active_areas if current_time - area[1] <= 2]
                ...
                active_area = count.most_common(1)[0][0]
                print("Most common active area:", active_area[0])
...
data_send_thread = threading.Thread(target=send_data_to_companion, args=(companion_sock,))
